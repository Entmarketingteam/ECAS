#!/usr/bin/env python3
"""
ContractMotion — Upload complete 6-email sequences for all 4 campaigns in single POST calls.
Smartlead's POST /sequences replaces all sequences on each call — must send all 6 at once.
Defense (3095136) already has all 6 — skipped.
"""
import os
import warnings
...
- USASpending NAICS 237130 (power/communications line construction) contract awards
...
The guarantee is grounded in the signal sources. County assessor land filings and utility interconnection requests for 50MW+ loads are public records that telegraph hyperscaler construction timelines with real precision.
...
I did pull the current hyperscale and commercial data center pipeline for your market. 11 active projects in permit or interconnection stages — 7 have not issued any public procurement notice yet.
...
We monitor these sources daily and surface projects at the pre-procurement stage.
...
The guarantee is grounded in the signals. CWSRF and DWSRF loan approvals are public records with predictable procurement timelines — once a loan is approved, the project is funded and the procurement window is measurable.
...
A mid-cap manufacturer filed an SEC 8-K last week disclosing a $340M facility expansion in the Southeast. EPC procurement typically opens 10–16 months after an 8-K disclosure at this scale.
...
The window to get on that list is the 12–18 months between an 8-K disclosure or industrial revenue bond filing and formal procurement — when the project is public but no one in EPC is watching it.
...
The guarantee is grounded in the signals. SEC 8-K filings and industrial revenue bond applications are public records with predictable procurement timelines — once a manufacturer discloses a capex project, the window is trackable.
...
# ============================================================
# EXECUTE — all 6 sequences per campaign in a single POST
# ============================================================

campaigns = [
    (3005694, "Power & Grid Infrastructure", POWER_GRID),
    (3040599, "Data Center & AI Infrastructure", DATA_CENTER),
    (3040600, "Water & Wastewater", WATER),
    (3040601, "Industrial & Manufacturing", INDUSTRIAL),
]

for cid, name, sequences in campaigns:
    print(f"\n{'='*60}")
    print(f"Campaign: {name} ({cid})")
    result = post_sequences(cid, sequences)
...
for cid in [3005694, 3040599, 3040600, 3040601, 3095136]:
    ...
    subjects = {s["seq_number"]: s["subject"] for s in seqs}
    status = "COMPLETE" if len(seqs) == 6 else f"INCOMPLETE ({len(seqs)}/6)"
    if len(seqs) != 6:
        all_complete = False
    print(f"\n  {names[cid]} ({cid}): {status}")
    for n in nums:
        print(f"    seq {n}: {subjects[n]}")

print(f"\n{'='*60}")
print(f"All campaigns complete: {all_complete}")
